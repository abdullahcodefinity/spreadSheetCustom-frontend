...
import { Key, Calendar, User, Edit, Trash2 } from 'lucide-react';
...
const KeyValueCard: React.FC<KeyValueCardProps> = ({ keyValue, onEdit, onDelete }) => {
  return (
      ...
      <div className="mb-4">
        <div className="flex flex-wrap gap-2">
          {keyValue.values.slice(0, 6).map((value, index) => (
            <span
              key={index}
              className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800"
            >
              {value}
            </span>
          ))}
